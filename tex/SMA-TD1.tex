...
Nous avons défini quelques actions de base pour nos philosophes. À chaque tour, selon l'état du philosophe et l'observation des données, une action est exécutée. Pour plus de clarté, nous avons représenté l'ensemble des actions implémentées sur une machine à états.\\
...
\draw[lien] (manger)--(fourchettes)node[midway,fill=white]{test};
\draw[lien] (penser)--(manger)node[midway,fill=white]{test};
\end{tikzpicture}
\end{center}
