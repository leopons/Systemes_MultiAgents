...
\begin{itemize}
\item Un certain nombre de philosophes se trouvent autour d'une table.
...
\begin{itemize}
\item Il peut penser, ce qui lui donne faim.
\item Il peut être dans un état de famine, s'il a faim et qu'il ne peut pas manger.
\item Il peut être en train de manger.
\end{itemize}
...
\item Quand un philosophe a faim, il attend que les fourchettes soient libres.
\end{itemize}
...
%------------------------------------------------
\pagebreak
\section{Plateforme SMA Synchrone}
...
\textcolor{red} {Notre plateforme} fonctionne de manière synchrone : le premier agent observe son environnement et effectue une action dont les critères d'exécution sont satisfaits par les données observées, puis c'est au tour du second et ainsi de suite. Arrivé au dernier agent, on recommence en repartant du premier.\\

Notre package contient les classes suivantes :

\begin{itemize}
\item \texttt{Agent} : Représente chaque agent du système, liés à un \texttt{Environnement}, à une liste d'actions possibles et à une boîte aux lettres (pour la réception des messages, détaillée plus loin).
\item \texttt{Environnement} : Représente l'environnement dans lequel évoluent les agents, contenant des \texttt{Donnee} et une liste d'agents \texttt{Agent}.
\item \texttt{Donnee} : Classe générique représentant les différentes données d'un problème.
\item \texttt{Action} : Classe abstraite destinée à représenter les différentes actions possibles, chaque action sera une sous-classe de celle-ci. Deux méthodes cruciales définissent une action : \texttt{check\_conditions} qui évalue les critères d'exécution de l'action, et \texttt{effectuer} qui exécute l'action proprement dit et modifie donc les données affectées par cette action.
\item \texttt{Message} : Les messages seront décrits dans un deuxième temps. Ils permettent la communication entre les agents.
\end{itemize}
...
\section{Application au problème du dîner des philosophes}
...
\draw[lien] (fourchettes)edge[loop above]node[midway,fill=white]{\begin{tabular}{c}Faim/Famine \\ !fourchettes \\ faim += deltaFFaim\end{tabular}}();
\draw[lien] (manger)edge[loop below]node[midway,fill=white]{\begin{tabular}{c}Manger \\ faim > 0 \\ faim += deltaFManger\end{tabular}}();
\draw[lien] (penser)edge[loop above]node[midway,fill=white]{\begin{tabular}{c}Penser \\ faim < seuilFaim \\ faim += deltaFPenser\end{tabular}}();
\draw[lien] (manger)--(penser)node[midway,fill=white]{\begin{tabular}{c}FinirManger \\ faim <= 0 \\ / \end{tabular}};
\draw[lien] (penser)--(fourchettes)node[midway,fill=white]{\begin{tabular}{c}FinirPenser \\ faim >= seuilFaim \\ / \end{tabular}};
\draw[lien] (fourchettes)--(manger)node[midway,fill=white]{\begin{tabular}{c}PrendreFourchettes \\ fourchettes \\ fourchettes=false \end{tabular}};
\end{tikzpicture}
...
L'intérêt bien sûr n'est pas de regarder ses philosophes manger mais bien de définir des constantes équilibrées pour étudier un système nuancé. Après quelques essais, nous avons décidé de travailler dans un premier temps avec xxxxx var. On remarquera que le choix de \texttt{deltaFFamine} ne joue que sur la variable de mesure \texttt{compteurFamine} et non sur le fonctionnement même du système.\\


\begin{figure}[!h]
\begin{center}
\includegraphics[width=\textwidth]{figure_1.png}
\caption{Évolution de la faim et de la pensée pour 9 philosophes}
\end{center}
\end{figure}



Le choix de \texttt{deltaFFamine} est donc un problème de convention. Pour le contexte donné en fig.X, on trouve par exemple xxxxx. On choisira \texttt{deltaFFamine}=60 pour les prochaines simulations.\\
